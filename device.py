...
def insert_random_data(table,deviceID):
    ...
    sql = f"INSERT INTO {table} (deviceID, timestamp, longitude, latitude) VALUES (%s, %s, %s, %s) ON DUPLICATE KEY UPDATE timestamp = %s, longitude = %s, latitude = %s;"
    val = (deviceID,time,x,y,time,x,y)
...
def read_latest(table,deviceID):
    # sql = '''SELECT * FROM gpsdata
    # ORDER BY timestamp DESC LIMIT 1;'''
    sql = f'''SELECT * FROM gpsdata
